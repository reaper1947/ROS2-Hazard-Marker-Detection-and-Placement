# 🤖 Node #2 — Hazard Marker Detection and Placement

**ROS2 Search & Navigation Challenge · COSC2781/2814 · RMIT University**
...
ระบบนี้ทำหน้าที่เป็น **Node #2** ในการแข่งขัน Search & Navigation Challenge ซึ่งหุ่นยนต์จะต้องสำรวจพื้นที่ที่ไม่รู้จักมาก่อน และค้นหาป้ายอันตราย 5 ป้ายที่ซ่อนอยู่ในพื้นที่นั้น

```
กล้อง RGB ──→ จำรูปป้าย ──→ คำนวณตำแหน่ง ──→ ปักหมุดบนแผนที่
...
## 🔧 สิ่งที่ต้องการก่อนรัน

### ฮาร์ดแวร์

| อุปกรณ์ | รายละเอียด |
|---------|------------|
| ROSBot Pro 3.0 / 2.0 | หุ่นยนต์หลัก |
| OAK-D Camera | กล้อง RGB สำหรับตรวจจับป้าย |
| RPLIDAR | เลเซอร์วัดระยะรอบตัว 360° |

### ซอฟต์แวร์ที่ต้องรันก่อน (prerequisite)

**ต้องรันทั้ง 3 ชั้นนี้ก่อนที่จะ launch Node #2:**

```
ชั้นที่ 1: ROSBot Hardware Driver
          └── publish: /oak/rgb/image_raw/compressed
          └── publish: /scan
          └── publish: /tf (base_link, odom)

ชั้นที่ 2: SLAM Toolbox + Nav2
          └── subscribe: /scan
          └── publish: /map
          └── publish: /tf (map → odom → base_link)

ชั้นที่ 3: ← Node #2 ของเรา (hazard_detector.launch.py)
```
...
snc_team/
├── launch/
│   └── hazard_detector.launch.py    ← ไฟล์หลักสำหรับรัน
├── snc_team/
│   ├── __init__.py
│   └── hazard_detector.py           ← โค้ด Node #2 หลัก
├── package.xml
└── setup.py
```
...
### ภาพรวม 3 Nodes ที่ทำงานพร้อมกัน
...
### Flowchart หลัก (hazard_detector node)
...
## 📖 ขั้นตอนโปรแกรมแบบละเอียด

### ขั้นที่ 0 — เตรียม prerequisite

ก่อน launch Node #2 ต้องมี 3 ชั้นนี้ทำงานอยู่แล้ว:
...
⚠️  ถ้า /tf map→base_link ไม่มี → Node #2 จะ warn ทุก callback
```
...
Launch file จะเปิด **3 nodes พร้อมกันทันที:**
...
|-------|-----------|----------|
| `/oak/rgb/image_raw/compressed` | กล้อง → repeater | ภาพดิบ BestEffort |
| `/oak/rgb/image_raw/compressed/repeat` | repeater → find_object_2d | ภาพ Reliable |

---

## ⚙️ Nodes ที่รันขึ้นมา
...
1. **ใช้ Laser ray เดียว** — ระยะที่วัดมาจาก laser index ตัวเดียว อาจคลาดเคลื่อนถ้าป้ายอยู่ขอบมุมกล้อง
2. **ไม่มี depth averaging** — ถ้า laser ray ตกในส่วน background (ไม่ใช่ตัวป้าย) ตำแหน่งจะผิด
3. **ขึ้นอยู่กับ SLAM accuracy** — ถ้าแผนที่ผิด ตำแหน่งป้ายก็ผิดตาม

### แนวทางปรับปรุง

```python
# แทนที่จะใช้ laser index เดียว ใช้ median ของ 5 rays รอบมุมนั้น
def get_laser_distance(self, angle):
    center_idx = int((angle - self.laser_angle_min) / self.laser_angle_increment)
    indices = range(max(0, center_idx-2), min(len(self.laser_ranges), center_idx+3))
    valid = [self.laser_ranges[i] for i in indices
             if not (math.isinf(self.laser_ranges[i]) or
                     math.isnan(self.laser_ranges[i]) or
                     self.laser_ranges[i] <= 0.05)]
...
RMIT IP — Not for public release  
COSC2781/2814 Programming Autonomous Robots · Semester 1 2026
